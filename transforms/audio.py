...
class ToMulaw(torch.nn.Module):
    ...
    def __init__(self, input_sample_rate: int = 16000):
        super().__init__()
        transform_list = [torchaudio.transforms.MuLawEncoding(
        ), torchaudio.transforms.MuLawDecoding()]
        if input_sample_rate != 8000:
            transform_list = [torchaudio.transforms.Resample(
                orig_freq=input_sample_rate, new_freq=8000)] + transform_list
...
class RandomSoxAugmentations(object):
    ...
    def __init__(self, input_sample_rate):
        ...
        self.augmentations = [
            ...
            ['pitch', '-q',
                str(uniform(*config.augmentations.pitch_range_in_cents))],
            ['tempo', '-q', '-s',
                str(uniform(*config.augmentations.tempo_range))],
            ['lowpass', '-2',
                str(uniform(*config.augmentations.lowpass_cutoff))]
        ]
        ...
        self.noise_file_list = [
            ...
            for path in open(
                os.path.join(
                    config.noises.noises_root, config.noises.noisefilelist),
                'r').read().split("\n") if path]
    ...
    def __add_noise(self, sample: torch.Tensor):
        ...
        if noise.size(1) >= sample.size(1):
            ...
            noise = torch.cat((noise.tile((1, times_to_tile)), noise[:, :(
                sample.size(1) - noise.size(1) * times_to_tile)]), dim=1)
    ...
    def __call__(self, sample: torch.Tensor):
        effects = random.sample(self.augmentations, k=randint(
            1, len(self.augmentations))) + [['rate', str(config.audio.model_sample_rate)]]
        augmented_sample, _ = apply_effects_tensor(
            sample, self.sample_rate, effects, channels_first=True)
...
class NoSoxAugmentations(object):
    ...
    def __call__(self, sample: torch.Tensor):
        augmented_sample, _ = apply_effects_tensor(
            sample, self.sample_rate, self.effect, channels_first=True)
        return augmented_sample
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        print(
            f"time taken for augmentation: {(time.process_time() - start) * 1000}ms, augmented shape: {augmented_waveform.shape}")
