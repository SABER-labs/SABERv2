...
class AggregatedEntropicTwins(pl.LightningModule):
    ...
    def __init__(self, num_samples):
        ...
        self.aggregated_ce_loss = AggregatedCrossEntropyLoss(stride=self.model_stride)
    ...
    def shared_step(self, batch):
        ...
        aggregated_l1_loss, aggregated_entropy_loss = self.aggregated_ce_loss(g1, g2, img1_len, img2_len)
        aggregated_total_loss = aggregated_l1_loss + (self.scale_entropy * aggregated_entropy_loss)
        train_loss = simclr_loss + (self.lambda_aggregated_loss * aggregated_total_loss)

        self.log('simclr_loss', simclr_loss, on_step=True, on_epoch=False)
        self.log('aggregated_l1_loss', aggregated_l1_loss, on_step=True, on_epoch=False)
        self.log('aggregated_entropy_loss', aggregated_entropy_loss, on_step=True, on_epoch=False)
        self.log('aggregated_total_loss', aggregated_total_loss, on_step=True, on_epoch=False)
