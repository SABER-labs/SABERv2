...
class Decoder(object):
    ...
    def __init__(self,blank_index=0):
        ...
        vocabs = [self.tokenizer.id_to_piece(id) for id in range(self.tokenizer.get_piece_size())]
        self.labels = vocabs
        space_index = len(vocabs)  # To prevent errors in decode, we add an out of bounds index for the space
        if '▁' in labels:
            space_index = vocabs.index('▁')
...
class GreedyDecoder(Decoder):
    ...
    def process_string(self, sequence, size, remove_repetitions=False):
        ...
        for i in range(size):
            char = sequence[i]
            if char != self.blank_index:
                # if this char is a repetition and remove_repetitions=true, then skip
                if remove_repetitions and i != 0 and char == sequence[i - 1]:
                    pass
                elif char == self.space_index:
                    string.append(' ')
                    offsets.append(i)
                else:
                    string.append(self.int_to_char[char])
                    offsets.append(i)
